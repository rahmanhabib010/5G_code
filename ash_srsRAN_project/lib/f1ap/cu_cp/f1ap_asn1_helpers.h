...
{
  // NSSAI
  if(request.s_nssai.has_value()){
    asn1_request->sst = request.s_nssai.value().sst;
    if(request.s_nssai.value().sd.has_value()){
      asn1_request->sd = request.s_nssai.value().sd.value();
    }
  }
