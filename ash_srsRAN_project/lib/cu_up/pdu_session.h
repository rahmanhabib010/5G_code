...
struct pdu_session {
  ...
  {
    // std::cout<<"xxxxxxxxxxxxxxxxxxxx| Creating PDU-session "<<std::endl;
    // int sd = -1;
    // if(session.snssai.sd.has_value()){
    //   sd = session.snssai.sd.value();
    // }
    // int sst = session.snssai.sst;
    // std::cout<<"sst = "<< sst<<"  , sd = "<<sd<<std::endl;
