...
{
  // std::cout<<"Recieved a Sctp-Payload"<<std::endl;
  ...
  if (rx_bytes == -1 && errno != EAGAIN) {
    ...
    // std::cout<<"Received "<<rx_bytes<<" bytes on SCTP socket"<<std::endl;;
    span<socket_buffer_type> payload(tmp_mem.data(), rx_bytes);
    if (msg_flags & MSG_NOTIFICATION) {
      // Received notification
        // std::cout<<"Received Notification on SCTP socket"<<std::endl;
      handle_notification(payload);
    } else {
      // Received data
      // std::cout<<"Received Data on SCTP socket"<<std::endl;
      handle_data(payload);
    }
  }
}
