...
#include "../../scheduler/policy/scheduler_time_rr.h"
#include<iostream>
...
void du_ue_ric_configuration_procedure::operator()(coro_context<async_task<ric_control_config_response>>& ctx)
{
  ...
  /* For Slice-Resource-Allocation-Change,
  The Current workflow is to directly call the scheduler (policy) to modify slice-config,
  Since, the actual/ideal pathway is unidentified (maybe because I am not from srsRan side, Since, their architecture is so big)
  With more time and investigations the above data will be sent from better channels.
  */

 if (request.per_slice_alloc.has_value()){
    ric_control_config_slice slice_policy = request.per_slice_alloc.value();
    bool out = true;
    for(slice_params curSlice: slice_policy.slice_config_list){
      out &= setSliceResourceAlloc(curSlice.sst, curSlice.sd, curSlice.partition);
    }
    std::cout<<"Slice-Resource-Allocations Status "<<out<<std::endl;
 }
