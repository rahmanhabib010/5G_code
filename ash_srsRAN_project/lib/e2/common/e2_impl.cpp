...
void e2_impl::handle_e2_setup_response(const e2_setup_response_message& msg)
{
  ...
  if (msg.success) {
    ...
    std::cout<<"Received E2 Setup Response message"<<std::endl;
    ...
    std::cout<<"E2 Setup Failure message received"<<std::endl;
...
{
  std::cout<<"Received RIC Control Request  e2_impl::handle_ric_control_request "<<std::endl;
  std::cout<<"RIC Message "<<msg->ri_cctrl_msg->to_string()<<std::endl;
  logger.info("Received RIC Control Request ");
  e2_ric_control_request request;
  request.request = msg;
  std::cout<<"Scheduling the operator: e2_ric_control_procedure "<<std::endl;
  async_tasks.schedule<e2_ric_control_procedure>(request, pdu_notifier, e2sm_mngr, logger);
  std::cout<<"The operator: e2_ric_control_procedure completed "<<std::endl;
}

void e2_impl::handle_e2_setup_failure(const e2_setup_response_message& msg)
{
  std::cout<<"Got e2_impl::handle_e2_setup_failure "<<std::endl;
  e2_message e2_msg;
  if (!msg.success) {
...
{
  std::cout<<"e2_impl::handle_ric_subscription_request"<<std::endl;
