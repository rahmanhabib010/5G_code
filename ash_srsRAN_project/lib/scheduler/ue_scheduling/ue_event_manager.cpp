...
#include<iostream>
#include "../policy/scheduler_time_rr.h" // This library is only included for deleteUeFromMap fxn
...
{
  std::cout<<"UE Creation Request Recieved by UE scheduler"<<std::endl;
...
void ue_event_manager::handle_ue_removal_request(du_ue_index_t ue_index)
{
  common_events.emplace(ue_index, [this, ue_index]() {
    ...
    std::cout<<"************* ------------- UE has Been Deleted "<<std::endl;
    deleteUeFromMap(rnti); // To delete the ue from the Map
    // Log event.
    ...
    if (h_dl != nullptr) {
      ...
      if (ack_value != mac_harq_ack_report_status::dtx) {
        ...
        if (ack_value == mac_harq_ack_report_status::ack){
          // notify to slice-class also
          if (not ue_db.contains(ue_cc.ue_index)) {
            logger.warning("Ue is not in ue_db for HARQ indication to slice-class");
            return;
          }
          uint8_t sst = ue_db[ue_cc.ue_index].sst;
          uint32_t sd = ue_db[ue_cc.ue_index].sd;

          logTransmittedBytes(sst, sd, tbs.value());
        // End
        }
