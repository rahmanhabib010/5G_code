...
bool ue_cell_grid_allocator::allocate_dl_grant(const ue_pdsch_grant& grant)
{
  ...
  // std::cout<<"In the end of dl-grant-allocation the crbs values are : "<<grant.crbs.start()<<" "<<grant.crbs.stop()<<std::endl;
  return true;
}
...
bool ue_cell_grid_allocator::allocate_ul_grant(const ue_pusch_grant& grant)
{
  ...
  if (not mcs_tbs_info.has_value()) {
    logger.warning(
        "rnti={:#x} Failed to allocate PUSCH. Cause: no MCS such that code rate <= 0.95 with this "
