...
void ue::handle_reconfiguration_request(const sched_ue_config_request& cfg)
{
  ...
  // Handle NSSAI Configurations
  if(cfg.sst.has_value()){
    sst = cfg.sst.value();
    if(cfg.sd.has_value()){
      sd = cfg.sd.value();
    }
    std::cout<<"Completed NSSAI Reconfiguring (At scheduler Level) UE : \nRNTI = "<<crnti<<" \t sst = "<<int(sst)<<" \t sd = "<<sd<<std::endl;
  }
}
