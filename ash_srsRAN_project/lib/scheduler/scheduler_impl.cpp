...
void scheduler_impl::handle_ue_creation_request(const sched_ue_creation_request_message& ue_request)
{
  ...
  std::cout<<"A UE Connection request is observe at cell "<<pcell_index<<std::endl;
...
void scheduler_impl::handle_ue_reconfiguration_request(const sched_ue_reconfiguration_message& ue_request)
{
  ...
  std::cout<<"A UE Reconfiguration request is observed at scheduler with following params"<<std::endl;
  int sst = (ue_request.cfg.sst.has_value()? ue_request.cfg.sst.value() : 0);
  int sd = (ue_request.cfg.sd.has_value()? ue_request.cfg.sd.value() : 0);
  std::cout<<"RNTI = "<<ue_request.crnti<<" \tsst = "<<sst<<" \t sd = "<<sd<<std::endl;
