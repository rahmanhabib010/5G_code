    ...
    coro_context<async_task<cu_cp_pdu_session_resource_setup_response>>& ctx)
{
  ...
  /*
  In order to print NSSAI recieved from Core
  for (auto pdu_item : setup_msg.pdu_session_res_setup_items) {
    int sd = 0;
    if (pdu_item.s_nssai.sd.has_value()) {
      sd = pdu_item.s_nssai.sd.value();
    }
    int sst = pdu_item.s_nssai.sst;
    std::cout << "From PDU Procedure: sst = " << int(sst) << "  , sd = " << sd << std::endl;
  }
  */
  ...
  // Added Code: To send sst-sd values to schedulers
  {
    // prepare UE Context Modification Request and call F1 notifier
    ue_context_mod_request.ue_index = setup_msg.ue_index;

    // Preparing UE Context for NSSAI values to scheduler
    if (setup_msg.pdu_session_res_setup_items.size() > 1) {
      std::cout << "WARNING | More than One PDU Session request for current ue| Unhandled Scenario" << std::endl;
    }
    for (auto pdu_item : setup_msg.pdu_session_res_setup_items) {
      ue_context_mod_request.s_nssai = pdu_item.s_nssai;
      break;
    }
