...
{
    ogs_info("Adding your ue 'ran_ue_add' function start");
    ...
    ogs_info("Ashish Logs| Adding your ue");
    stats_add_ran_ue();
...
void amf_ue_new_guti(amf_ue_t *amf_ue)
{
    ...
    ogs_info("New GUTI is getting allocated to {%d}", amf_ue->next.guti.m_tmsi);
}
        ...
        ogs_plmn_id_t *served_plmn_id, ogs_s_nssai_t *s_nssai)
{
    ...
    for (i = 0; i < amf_self()->num_of_plmn_support; i++) {
        ...
        for (j = 0; j < amf_self()->plmn_support[i].num_of_s_nssai; j++) {
            /* Compare S-NSSAI */
            ogs_info("Matching PLMN Found with sst %d and sd %d ",amf_self()->plmn_support[i].s_nssai[j].sst, amf_self()->plmn_support[i].s_nssai[j].sd.v );
            ogs_info("GNB Side values sst = %d & sd %d", s_nssai->sst, s_nssai->sd.v);
