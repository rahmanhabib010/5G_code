...
static bool served_tai_is_found(amf_gnb_t *gnb)
{
    ...
    ogs_info("Gnb Served TAI index length are  : %d", gnb->num_of_supported_ta_list);
    for (i = 0; i < gnb->num_of_supported_ta_list; i++) {
        for (j = 0; j < gnb->supported_ta_list[i].num_of_bplmn_list; j++) {
            ogs_5gs_tai_t tai;
            memcpy(&tai.plmn_id,
            ...
            ogs_info("    TAC[%d]", gnb->supported_ta_list[i].tac.v);
            ogs_info("    PLMN_ID[MCC:%d MNC:%d]",
                    ogs_plmn_id_mcc(&gnb->supported_ta_list[i].
                        bplmn_list[j].plmn_id),
                    ogs_plmn_id_mnc(&gnb->supported_ta_list[i].
                        bplmn_list[j].plmn_id));
            ogs_info("    SERVED_TAI_INDEX[%d]", served_tai_index);
...
static bool s_nssai_is_found(amf_gnb_t *gnb)
{
    ...
    for (i = 0; i < gnb->num_of_supported_ta_list; i++) {
        for (j = 0; j < gnb->supported_ta_list[i].num_of_bplmn_list; j++) {
            ogs_info("Nunber of s_nssai on gnb side : %d ", gnb->supported_ta_list[i].
