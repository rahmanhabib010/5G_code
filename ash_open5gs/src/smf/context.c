...
smf_sess_t *smf_sess_add_by_apn(smf_ue_t *smf_ue, char *apn, uint8_t rat_type)
{
    ...
    // sprintf(pdu_ip,"%s", OGS_INET_NTOP(&sess->ipv4->addr, buf1));
    // sessionAdd(smf_ue->supi, pdu_ip, "Added-APN ");
...
smf_sess_t *smf_sess_add_by_psi(smf_ue_t *smf_ue, uint8_t psi)
{
    ...
    // char buf1[OGS_ADDRSTRLEN];
    // sprintf(pdu_ip,"%s", OGS_INET_NTOP(&sess->ipv4->addr, buf1));
    // sessionAdd(smf_ue->supi, pdu_ip, "Added-PSI ");
...
void smf_sess_remove(smf_sess_t *sess)
{
    ...
    char pdu_ip[64];
    sprintf(pdu_ip,"%s", OGS_INET_NTOP(&sess->ipv4->addr, buf1));
    sessionUpdate(smf_ue->supi, pdu_ip, false);
